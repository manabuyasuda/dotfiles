...
CSSファイルの統計情報を分析するツール。ファイルサイズ、ルール数、セレクター数、宣言数、メディアクエリ数、ユニークな色数、フォントサイズ数、フォントファミリー数、詳細度の分布などの指標を一覧表示する。
...
ファイルサイズ、ルール数、セレクター数、宣言数、メディアクエリ数、ユニークな色数、フォントサイズ数などの統計情報をテーブル形式で一覧表示し、CSSの肥大化や複雑度を検知する。
...
複数のCSSファイルの統計情報をJSON形式で出力し、ファイルサイズやセレクター数などの指標を比較する。ライブラリの選定やビルド設定の検証に使える。
...
リファクタリング前後のJSON出力を比較して、ファイルサイズの削減、セレクター数や宣言数の変化を定量的に評価する。
