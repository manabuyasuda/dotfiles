...
| コマンド | 説明 |
| --- | --- |
| `lhci collect` | Lighthouseを実行し、結果をローカルフォルダーに保存する |
...
`collect`で収集した結果をHTMLレポートとしてブラウザで開く。CIへアップロードせず、手元で結果を確認したい場合に便利。
...
`--no-lighthouserc`オプションで自動読み込みを無効にし、`--config`オプションで別のファイルも指定できる。
