...
VercelプラットフォームのCLIツール。ローカルからのデプロイ、環境変数の管理、プロジェクト設定などを行える。
...
ブランチごとにプレビュー URLが発行され、レビュアーがデプロイ結果を確認できる。
...
Vercelのサーバーレス関数やルーティング設定をローカルで再現し、デプロイ前に動作確認する。
...
プロダクション環境で問題が発生したとき、以前の正常なデプロイメントへ即座に戻す。再ビルドなしで切り替わるため、復旧が速い。
