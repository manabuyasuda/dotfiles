...
| ツール | 説明 | インストール |
|---|---|---|
| [Socket](socket.md) | npmパッケージのサプライチェーンリスクを検出。悪意あるパッケージやタイポスクワッティングを識別する | npm（mise） |
| [sfw（Socket Firewall）](sfw.md) | npm・pnpm・pip など主要パッケージマネージャーをラップし、不審なパッケージのインストールをネットワーク層で遮断する | npm（mise） |
...
| ツール | 説明 | インストール |
|---|---|---|
| [Docker（OrbStack）](docker.md) | DockerコンテナーのランタイムとしてOrbStackを使う。Docker Desktopの軽量な代替で、`docker compose` コマンドのバックエンドとして動作する。導入手順は本文を参照 | cask |
