#!/usr/bin/env bash
# adapters/plan-guard.sh — plan-guard の Cursor アダプタ
...
INPUT=$(cat)
CLAUDE_HOOK="$(cursor_io_claude_pre_tool_use_hook plan-guard.sh)"

if [[ ! -x "$CLAUDE_HOOK" ]]; then
  cursor_io_fail_open_missing_hook "plan-guard adapter" "$CLAUDE_HOOK"
fi
