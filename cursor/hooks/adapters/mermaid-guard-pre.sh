#!/usr/bin/env bash
# adapters/mermaid-guard-pre.sh — mermaid-guard (pre) の Cursor アダプタ
...
INPUT=$(cat)
CLAUDE_HOOK="$(cursor_io_claude_pre_tool_use_hook mermaid-guard.sh)"

if [[ ! -x "$CLAUDE_HOOK" ]]; then
  cursor_io_fail_open_missing_hook "mermaid-guard-pre adapter" "$CLAUDE_HOOK"
fi
