...
>
> サンプル内の`@...`はバージョン参照の省略です。実際にはリリースのcommit hashに固定してください（「Actionバージョンをcommit hashで固定する」を参照）。モデル識別子（`claude-sonnet-4-6`など）も確認日時点のものです。
...
`claude-code-action`では多くの設定をCLI引数（`claude_args`）としてまとめて渡します。`claude-code-base-action`も`claude_args`を受け付けます。一方、`track_progress`（進捗コメント）と`use_sticky_comment`（コメントの集約）は`claude-code-action` v1専用のinputで、base-actionにはありません。
...
### pull_request を起点にする場合

PRの開閉やプッシュを起点に自動レビューを回す場合は、`pull_request` / `pull_request_review`を使います。ただしフォークからの`pull_request`では`GITHUB_TOKEN`がread-onlyになり、コメント投稿やコミットが失敗します。外部コントリビューターのPRを扱うときは、secretsが渡る`pull_request_target`を安易に使わず、メンバー限定で実行を絞るなどの対策を取ります（「フォークPRと信頼できない入力を前提にする」を参照）。
...
- トリガーを限定する: 特定ラベル・特定パス・`@claude`メンションのみなど、`on:`と`if:`で起動条件を狭めます。
- 1実行の上限を決める: `claude_args`の`--max-turns`（ターン数）と`--max-budget-usd`（API利用額。printモード）で打ち切ります。
- 多重起動を抑える: concurrencyで同一Issue / PRの並行実行を止めます（「Concurrency制御」を参照）。
...
ラベルトリガーを使う場合、ラベルを付けた人（write権限あり）とIssue本文を書いた人が別人になり得ます。Issue作成者の権限を実行時にAPIで確認します。
...
ホワイトリスト設計の理由は次のとおりです。Issue本文がGitHub Appやtrusted botによって自動生成される場合、権限APIには登録されていないことがあります。ボット名を明示的に許可しておくと、意図せず弾かれるのを防げます。

### スコープ限定

PRに含まれるファイル変更が特定のディレクトリ内に収まっているかを確認してから実行すると、関係のない変更に反応するリスクを下げられます。
...
ジョブ側で必要なものだけを明示します。トップレベルで`permissions: {}`を設定してデフォルト権限を封じ、ジョブごとに必要最小限を付与します。
...
`Bash`だけを許可するとファイル読み書きができない一方でシェル操作は何でも通る、という歪な状態になります。

実装タスクに必要な最小セットは次のとおりです。
...
MCPツールは`mcp__<server>__<tool>`形式で個別に許可できます。
...
同一Issue / PRへの並行実行はリソースのムダ遣いになり、競合状態を引き起こすこともあります。
...
`cancel-in-progress`の選び方は次のとおりです。
...
ワークフローレベルではなくジョブレベルに設定するのは、ワークフローレベルだと`if`条件でスキップされたときもキャンセルが発動してしまうためです。

---

## 認証設計

認証方式は次のとおりです（順序は計測データではなく、`ANTHROPIC_API_KEY`を公式の既定とし、以降を本番での推奨順に並べたものです）。いずれも両Actionで利用できます（Microsoft Foundryを使う`use_foundry`はbase-actionのみ）。
...
デフォルトの`GITHUB_TOKEN`は`actions/checkout`が事前に使っているため、Claudeが作るコミットやPRがCIをトリガーできないケースがあります。GitHub App Tokenを使うとCIが正常にトリガーされます。App Tokenは`env`ではなく`github_token`というinputで渡します（自前のGitHub Appを使うときだけ指定し、通常は不要です）。
...
- uses: anthropics/claude-code-action@...
  with:
    anthropic_api_key: ${{ secrets.ANTHROPIC_API_KEY }}
    github_token: ${{ steps.app-token.outputs.token }}  # 自前のAppを使うときだけ指定
```
...
関係のないコードをチェックアウトするとインストール・ビルドが遅くなり、コンテキストにも余計なものが混入します。
...
`sparse-checkout-cone-mode: false`が必要なのは次の理由からです。cone modeはディレクトリ単位の高速マッチングですが、`systems/your-product`以外の設定ファイルが除外されてしまうケースがあります。非cone modeにするとglobパターンが正確に機能します。

### `persist-credentials: false` の推奨

デフォルトではcheckout後にgit credentialとして`GITHUB_TOKEN`が残ります。Claudeがpush等を行う場合、意図しないトークンで操作されるリスクを避けるため、`false`にして明示的に`GH_TOKEN`環境変数で渡す方が安全です。
...
Claudeの実行ステップで`continue-on-error: true`にした場合、`failure()`だけでは失敗を検知できません。`steps.<id>.outcome == 'failure'`も条件に加えます。
...
CI上ではsparse-checkoutでCLAUDE.mdを含めていなかったり、hooksが動かなかったりする場合があります。重要な前提はプロンプト内で明示的に伝えると一貫した動作が得られます。
...
コメント内のキーワードでモデルを切り替えると、通常は軽量モデルを使いながら必要なときだけ高性能モデルを使えます。
...
Claudeが途中経過を複数コメントで投稿するとPRが散らかります。sticky comment（同じコメントを更新し続ける）で集約します。`track_progress` / `use_sticky_comment`は`claude-code-action` v1専用のinputで、base-actionにはありません。
...
依存インストール等で時間がかかる前に「受け付けた」コメントを先に投稿すると、ユーザーは応答が始まったことを確認できます。
...
タグ（`@v1`）はミュータブルで上書きされる可能性があります。commit hashで固定すると予期しない変更を防げます。
...
確認日（2026-06-15）時点で到達を確認した出典です。実運用の事例は別ファイル[Claude Code GitHub Actions事例集](./claude-code-github-actions-cases.md)を参照してください。
