...
- スキルの自動起動の可否は `skillOverrides`（settings.json）で一元管理する。descriptionに「積極的に起動」などの自動起動を促す表現を書くのは、自動起動を許可しているスキルに限る。明示専用（`user-invocable-only`）なのにdescriptionが自動起動を促す矛盾は、検証スクリプトが検出する。
