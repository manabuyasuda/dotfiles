...
| `x-argument-map` | 「何が言いたいのか分からない」「論点を整理して」「argument-map」 | 日本語のMarkdown文書の主張・根拠を段落単位で洗い出し、扱いをユーザーに質問して完成稿と変更一覧を提示（ユーザーが明示的に起動した場合のみ動作） |
...
| `decompose` | `@decompose` | 計画の作成・更新時と実行前に、各ステップを独立して確認・コミットできる実行単位へ分解する |
...
プラグインは`plugin.json`で`defaultEnabled: false`を宣言できます。既定でオフのプラグインも、ここで`true`を指定すれば有効になります。依存先のプラグインは自動で有効になるため、`enabledPlugins`には直接使うものだけを書けば十分です。一時的にオン・オフを切り替えたい場合は`/plugin`または`claude plugin enable <name>`を使います。
...
オートモードは、各ツール呼び出しを分類器（サーバー側の別モデル）が事前評価し、不可逆・破壊的・環境外を狙う操作に加え、リクエスト範囲を超えるエスカレーションやインジェクション駆動とみられる操作をブロックして権限プロンプトを省く。Anthropicは `--dangerously-skip-permissions` より安全な選択肢と位置づけている。ただし分類器は確率的で偽陰性が残るため過信はできない（詳細は [SECURITY.md](SECURITY.md) を参照）。リサーチプレビューのため、本リポジトリではデフォルトで有効にしない。必要なときにセッション単位・案件単位で有効化する。
...
`PreToolUse(Bash)`に登録するガードは、`claude --bg --exec '<command>'`と`! <command>`からは発火しません。これらはコマンドをシェルへ直接渡すため、`bash-guard.sh`・`dangerous-guard.sh`・`push-to-main-guard.sh`・`check-gh-account.sh`がすべて素通りします。詳細と運用上の方針は[SECURITY.md §6 バックグラウンドシェルはガードを通らない](SECURITY.md#バックグラウンドシェルclaude---bg---exec--はガードを通らない)を参照してください。
...
Q3: 並列化・ファンアウトが効くか（複数視点でのレビュー・複数案の同時生成・大量ファイルの一括処理など）？  
→ YES: `/workflows` を検討する（数十〜数百のサブエージェントを動的にオーケストレートする。実行状況は `/workflows` で確認）  
→ NO: skill / agentで逐次実行する

`/workflows`はサブエージェントを並列起動するため、1リクエストあたりのトークン消費が逐次実行の約3倍以上になります。「複数視点で同時に見たい」「複数案を並べて比較したい」場面に限定し、無意識に選ばないようにしてください。
...
Opus 4.8以降ではlean system prompt（軽量なシステムプロンプト）が既定となり、`CLAUDE.md`の指示は冗長になるほど効きが弱くなります。100行ルールはlean system prompt前提でも意図どおりに効かせるための基準として設定しています。
