...
- `.mcp.json`を含むリポジトリを開いたときは、`claude mcp list`で「⏸ Pending approval」の表示を確認し、サーバーの素性を検証してから`/mcp`で個別に承認します。v2.1.154以降、パイプ出力時も承認していないサーバーは明示され、自動では承認されません
...
v2.1.154で追加された`claude --bg --exec '<command>'`および`claude agents`内の`! <command>`は、コマンドをシェルへ直接渡して実行します。Claude CodeのBashツールを経由しないため、`PreToolUse(Bash)`に登録された`bash-guard.sh`・`dangerous-guard.sh`・`push-to-main-guard.sh`・`check-gh-account.sh`はいずれも発火しません。実機検証（2026-06-25）では、`claude --bg --exec 'rm -rf <ファイル>'`が`dangerous-guard.sh`を素通りして対象ファイルを削除しました。

このため、対話セッションで`rm -rf`がdenyされていても、`claude --bg --exec 'rm -rf <パス>'`は実行されます。`git push origin main`、`gh secret set`、`chmod 644 .claude/settings.json`など、Bashツール経由ならガードで止まる操作も同様にすり抜けます。

運用上の方針は以下の通りです。

- バックグラウンドシェルを実行する前に、コマンド文字列を自分の目で確認します（ガードは効きません）
- 間接プロンプトインジェクションやMCPサーバーが侵害された経路でバックグラウンドシェルを起動されると、ガードを通らずに任意コマンドを実行されます。Claudeが`claude --bg --exec`や`!`を提案してきた場合は、対話セッションで同じコマンドを実行する場合よりも慎重に内容を確認します
- 機密性の高い作業中は、自分以外（Claudeを含む）にバックグラウンドシェル経由のコマンド実行を任せません

### 7. データ持ち出し（Exfiltration）

インジェクション経由で秘密情報が外部に送られるリスク。`gh pr comment`や`gh api`のPOST経由でコードや環境変数を書き出せる経路がある。

v2.1.154以降、Auto-modeの分類器がリポジトリ全文の一括転送など大量データ持ち出しの検出を強化しています。ただし分類器はあくまで補助で、検出は確率的にすり抜けが残ります。主軸はLayer 1（permissionsのdenyで秘密ファイルへのReadを止める）とsandbox（`allowedDomains`で通信先を制限する）に置き、Auto-modeの強化を理由に主軸を緩めません。
