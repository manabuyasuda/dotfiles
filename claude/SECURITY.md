...
AIコーディングエージェントは通常のプログラムと異なり、読み込んだコンテンツが命令として解釈されるリスクがある。エージェントに渡す権限の範囲と、権限を強制する仕組みの設計がセキュリティの核心である。
...
- **Cursorの自動実行モード** — インジェクション成功率が攻撃手法によって41〜84%に達したとの報告あり（出典: [arXiv:2509.22040](https://arxiv.org/abs/2509.22040) "Your AI, My Shell"）。特定のバージョンの脆弱性ではなく自動実行モード全般の設計上のリスクのため、モードを有効にする際は注意が必要
...
- `.mcp.json`を含むリポジトリを開いたときは、`claude mcp list`で「⏸ Pending approval」の表示を確認し、サーバーの素性を検証してから`/mcp`で個別に承認する。v2.1.154以降、パイプ出力時も承認していないサーバーは明示され、自動では承認されない
...
- **Claude Codeでの現状** — 長期メモリ機能（`~/.claude/projects/*/memory/`等）はユーザーが直接管理する。組み込みのポイズニング対策はないため、運用でカバーする
...
対策
- **git管理** — hooks・settingsの変更はPRと`git diff`で事後検知する。Claude Codeは`claude/hooks/`と`claude/settings.json`、Cursorは`cursor/hooks/`（`~/.cursor/hooks`へsymlink）をgit管理する。カナリア（hooksへの`ask`）は廃止した（自己修復を阻害するデメリットが大きいため）
...
#### バックグラウンドシェル（`claude --bg --exec` / `!`）はガードを通らない

v2.1.154で追加された`claude --bg --exec '<command>'`および`claude agents`内の`! <command>`は、コマンドをシェルへ直接渡して実行する。Claude CodeのBashツールを経由しないため、`PreToolUse(Bash)`に登録された`bash-guard.sh`・`dangerous-guard.sh`・`push-to-main-guard.sh`・`check-gh-account.sh`はいずれも発火しない。実機検証（2026-06-25）では、`claude --bg --exec 'rm -rf <ファイル>'`が`dangerous-guard.sh`を素通りして対象ファイルを削除した。

このため、対話セッションで`rm -rf`がdenyされていても、`claude --bg --exec 'rm -rf <パス>'`は実行される。`git push origin main`、`gh secret set`、`chmod 644 .claude/settings.json`など、Bashツール経由ならガードで止まる操作も同様にすり抜ける。

運用上の方針は次のとおりである。

- バックグラウンドシェルを実行する前に、コマンド文字列を自分の目で確認する（ガードは効かない）
- 間接プロンプトインジェクションやMCPサーバーが侵害された経路でバックグラウンドシェルを起動されると、ガードを通らずに任意コマンドを実行される。Claudeが`claude --bg --exec`や`!`を提案してきた場合は、対話セッションで同じコマンドを実行する場合よりも慎重に内容を確認する
- 機密性の高い作業中は、自分以外（Claudeを含む）にバックグラウンドシェル経由のコマンド実行を任せない
...
| `ANTHROPIC_BASE_URL`の変更 | Claude Codeの通信先を偽サーバーに向ける攻撃（APIキーが第三者サーバーに送信される） |

### 8. Claude Code固有の脆弱性（hooksファイル自体の攻撃）
...
オートモードは、各ツール呼び出しを分類器（サーバー側の別モデル）が事前評価し、不可逆・破壊的・環境外を狙う操作や、リクエスト範囲を超えるエスカレーション・インジェクション駆動とみられる操作をブロックする。この分類器は3層防御を置き換えない。決定的なLayer 1（permissions）とLayer 2（hooks）の間に挟まる確率的な補助層という意味で、ここではLayer 1.5と呼ぶ。分類器自体はプロンプトインジェクション検知とトランスクリプト分類器の2系統で構成される（参考文献の「2層防御」は同じ対象を別の解像度で見たもの）。
...
- `permissions.deny` は分類器より先に評価される。`.env`・`~/.ssh`・シークレット書き込みなどのdenyはオートモードでも有効に機能する。
...
- オートモードのデフォルトのsoft_denyは、ハーネス自身の設定（`settings*.json`・`CLAUDE.md`・`claude/hooks/`・`cursor/hooks/`）の変更をブロックする（`Self-Modification`）。これは §6「ガードの無効化」やCVE-2025-59536に対する確率的な追加防御になる。
...
>
> 読み取り（Read）および編集（Edit）の拒否ルールは、Claudeに組み込まれているファイル操作ツールに対して適用されるものであり、Bashサブプロセスには適用されない。例えば、`Read(./.env)`の拒否ルールを設定しても、Readツールによる読み取りはブロックされるが、Bash上で`cat .env`を実行することは防げない。すべてのプロセスに対して特定のパスへのアクセスをOSレベルで制限したい場合は、サンドボックスを有効にする。
...
| `.claude/hooks/` / `.claude/settings.json` / `cursor/hooks/` | — | ハーネス自身の設定ファイル。git管理で変更を追跡する（カナリアaskは廃止） | — |
...
| 外部リポジトリの README / CLAUDE.md / `.claude/settings.json`（clone） | cloneしてClaude Codeで開くと`.claude/settings.json`のhooksが自動実行される（CVE-2025-59536）。ファイル内のテキストにインジェクションが仕込まれている可能性もある |
