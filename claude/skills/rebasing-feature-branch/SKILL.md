...
インストールされていれば、importの順序・JSONキー追加など構造的なコンフリクトを自動解消する。

```bash
git diff --name-only --diff-filter=U | xargs -I{} mergiraf solve {}
```

実行後、残っているコンフリクトマーカーを確認する。

```bash
git diff --name-only --diff-filter=U
```

すべて解消されていれば手順4（コードを修正〜）をスキップして `git add` と `git rebase --continue` に進む。コンフリクトマーカーが残っている場合は手順3に進む。
