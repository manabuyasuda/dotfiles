...
`vercel-react-best-practices` スキルのルールも参照して、Vercel公式のベストプラクティスに照らした指摘を加える。

1. **バグ・ロジックの問題**
   - オフバイワンエラー、null/undefined参照、境界値の見落とし
   - 非同期処理の競合状態・エラーハンドリングの漏れ
   - イベントハンドラーの二重実行・クリーンアップ漏れ（useEffect等）

2. **コンポーネント設計・アーキテクチャ**
   - 単一責務になっているか（UIとロジックが混在していないか）
   - propsのインターフェイスは適切か（過剰・不足・命名の一貫性）
...
4. **パフォーマンス**
   - 不要な再レンダリングが発生していないか（memo/useMemo/useCallbackが必要な箇所にのみ適用されているか）
...
5. **型安全性**
   ...
   - イベントハンドラー・APIレスポンスの型が正確に定義されているか
