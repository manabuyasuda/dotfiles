---
name: x-figma-implement
description: >
  x-figma-extractが生成したマッピングファイルとスクリーンショットをもとに、Figmaのデザインに合わせてコンポーネントやページを実装するスキルです。
  ...
  またはx-figma-extractの直後に実装フェーズへ進むときに必ず起動してください。
  Figma URLが渡されて実装まで一気に依頼された場合は、まずx-figma-extractで取得・マッピングを終えてから本スキルを起動します。
  デザイン取得・トークン引き当て・マッピングファイル作成自体は本スキルでは扱いません。それらはx-figma-extractで実行してください。
allowed-tools:
  - Bash
  - Read
  - Write
...
このスキルはx-figma-extractの成果物を入力として、コンポーネント単位でデザインを再現します。アーキテクチャ設計・状態管理・ルーティング・データフェッチングは扱いません。

## x-figma-extractとの連動

Figma URLや「このデザインを実装して」のように依頼された場合、本スキルは単体では動きません。以下の順で連動します。

1. x-figma-extractが起動します。Figma MCPからデータを取得し、マッピングファイルとスクリーンショットを保存します
2. x-figma-extractの完了後、x-figma-implement（本スキル）が起動します。Step 1の入力チェックから実装と検証まで進めます

ユーザーから「Figma URL → 実装まで」とまとめて依頼されたときは、まずx-figma-extractスキルを呼び出して成果物を揃え、続けて本スキルへ進みます。マッピングファイルがすでに存在する場合は本スキルから開始します。
...
x-figma-extractの成果物が揃っているか確認します。マッピングファイル・スクリーンショットの正式な保存先パスはx-figma-extractスキルの`references/project-save-format.md`から参照してください。x-figma-extract側で保存形式が変更されている可能性があるため、毎回ここから読み取ります。

| 確認項目 | 方法 |
|---|---|
| マッピングファイルが存在する | x-figma-extract側の保存形式に従ったパスを`ls`で確認 |
| スタイル表のトークン列にすべて値が入っている | ファイルを開いて目視確認 |
| テキスト・アイコン・画像の各表が記録されている | マッピングファイルのテンプレートと照合 |
| スクリーンショットが保存されている | x-figma-extract側の保存形式に従ったパスを`ls`で確認 |

1つでも欠けていればx-figma-extractに戻ります。空欄のままトークンを推測して実装しないでください。
...
| レイアウト算出値（親Xの中で`x=Y` `w=Z`） | Step 7（`getBoundingClientRect`での照合） |
...
3回目は実装済みコードがある場合に限り、対象コンポーネントのTSX・CSSファイルを読み、「確認するHTML要素」に挙げられた違反パターンが実装に混入していないか確認します。マッピングファイルや既存実装に引っ張られず、ルール本体の基準のみで判断します。

いずれかで「適用する」となったルールは適用します。3回すべてで「適用しない」となったルールのみスキップします。
...
マッピングファイルのパスと記録形式は`references/project-save-format.md`（x-figma-extract側）を参照します。記録先のマッピングファイルがどれか不明な場合は、`AskUserQuestion`で必ずユーザーに確認します。
...
1. x-figma-extractのマッピングファイルで`### スクリーンショット`の「Figmaフレーム寸法」を確認します
