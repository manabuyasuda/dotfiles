...
出力されたペアのうち、`CHANGED_FILES`に含まれないファイルが片方に現れた場合は「変更漏れ候補」としてStep 5の`[設計]`レビューで確認する。
...
- Temporal Couplingで未含ファイルが検出された場合: 常にペアで変更されるはずのファイルが今回のPRに含まれていない場合、変更の見落としか意図的な除外かを確認し、意図的でなければ指摘する
