...
git履歴はファイルの「設計的な不安定さの蓄積」を示す。変更ファイルがもともとホットスポットかどうかを把握することで、レビューの深度・`[設計]`指摘の優先度を正しく判断できる。

各分析には一定のコミット数が必要。最低ラインに満たない場合はその分析をスキップし、最低ライン以上・信頼できるライン未満の場合は結果に `[低精度]` を付加して出力する。
...
変更頻度が高く行数も多いファイルは「複雑なのに頻繁に触られる」ホットスポット。今回の変更が問題をさらに悪化させていないかを重点的にレビューする判断材料にする。
...
削除率が50%超のファイルは「書いては消す」が繰り返されている設計の不安定シグナル。今回の変更がその傾向をさらに強めていないかを確認する。

最低ライン（10回）以上のファイルのみを対象にし、信頼できるライン（20回）未満のファイルは注意付きで出力する。
...
出力形式: `削除率 総変更行数 削除行数 ファイルパス`

#### Temporal Coupling（暗黙の結合検出）

変更ファイルを起点に「常にペアで変更されるファイル」を検出する。今回のPRに含まれていないファイルがペアとして出現した場合、変更の見落とし（または暗黙の結合）の可能性がある。静的依存がなくても行動的に結合しているファイルを発見できる。
...
出力されたペアのうち、`CHANGED_FILES`に含まれないファイルが片方に現れた場合は「変更漏れ候補」として Step 5 の`[設計]`レビューで確認する。
...
- ホットスポットスコアが高いファイルへの変更: 変更頻度が高く行数も多いファイルに新たにロジックを追加している場合、責務が肥大化していないかを確認する。分割・抽出を提案する判断材料にする
- 書き換え率が50%超のファイルへの変更: 今回の変更が同じ「書いては消す」パターンの繰り返しになっていないかを確認する。設計レベルで解決できる根本原因がないかを検討する
- Temporal Coupling で未含ファイルが検出された場合: 常にペアで変更されるはずのファイルが今回のPRに含まれていない場合、変更の見落としか意図的な除外かを確認し、意図的でなければ指摘する
...
git履歴コンテキストの分析結果から、レビューに影響する情報のみを出力する。シグナルが何もない場合は省略してよい。
...
- <ファイルA> <-> <ファイルB>: <N>回のコミットで共変更 ※今回のPRに <ファイルB> が含まれていない
```
