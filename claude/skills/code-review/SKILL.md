...
ブランチ全体の差分を取得します（コミット済み変更）。

```bash
git diff origin/$BASE_BRANCH...HEAD
```

未コミット変更を取得します（あれば含めます）。
...
ファイルパスが入力された場合はそのファイルを読み、URLが入力された場合は内容を取得して、変更の意図・制約・設計方針をコンテキストに加えます。
...
変更ファイルを起点に「常にペアで変更されるファイル」を検出します。今回のPRに含まれていないファイルがペアとして出現した場合、変更の見落とし（または暗黙の結合）の可能性があります。静的依存がなくても行動的に結合しているファイルを発見できます。
...
出力されたペアのうち、`CHANGED_FILES`に含まれないファイルが片方に現れた場合は「変更漏れ候補」としてStep 5の`[設計]`レビューで確認します。
...
「コードレビュー」はClaude Codeが差分を直接読んで行う評価を指します。Step 4の「自動解析」（CLIツール出力）とは区別して使用します。
...
Step 4で取得した全ツール・全スキルの出力から指摘を抽出し、同一または実質同じ内容はひとつの項目に集約します。各項目には該当するカテゴリを複数付与してかまいません。出力がない場合は省略します。
...
差分・スコープファイルを読み、自動解析では検出できない問題を中心にコードレビューを実施します。条件に該当するカテゴリはスキップします。読み込んだルールを基準に3回繰り返します。1回目の指摘を反映した状態で2回目を実施し、2回目の指摘を反映した状態で3回目を実施します。
...
Step 4の自動解析で検知できないもの、およびプロジェクト固有の判断が必要なものに絞って確認します。
...
- Temporal Couplingで未含ファイルが検出された場合 — 常にペアで変更されるはずのファイルが今回のPRに含まれていない場合、変更の見落としか意図的な除外かを確認し、意図的でなければ指摘します
...
- PRの差分が大きい場合は、変更ファイルをカテゴリ別に整理して段階的にレビューします
