...
### Phase 4: コードレビュー（`x-thorough-code-review`スキル）

`x-thorough-code-review`スキルを起動してローカルブランチの変更をレビューします。
...
must/shouldの修正が発生するたびに`x-thorough-code-review`スキルをフルで再実行します。修正が連鎖して別の箇所に影響していることがあるためです。再レビューの差分は小さいので実際の作業量は少なくなります。
...
`x-thorough-code-review`スキルが起動できない場合は、その旨をPhase 6の報告に記載してスキップします。
