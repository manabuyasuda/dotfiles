...
### Phase 4: コードレビュー（`code-review-engine`エージェント）

`code-review-engine`エージェントを起動してローカルブランチの変更をレビューします。
...
must/shouldの修正が発生するたびに`code-review-engine`エージェントを再実行します。修正が連鎖して別の箇所に影響していることがあるためです。再レビューの差分は小さいので実際の作業量は少なくなります。
...
`code-review-engine`エージェントが起動できない場合は、その旨をPhase 6の報告に記載してスキップします。
...
- `code-review-engine`エージェントは省略しません — 起動できない場合のみ理由を明記してスキップします
