...
### Phase 4: コードレビュー（`thorough-code-review`スキル）

`thorough-code-review`スキルを起動してローカルブランチの変更をレビューします。
...
must/shouldの修正が発生するたびに`thorough-code-review`スキルをフルで再実行します。修正が連鎖して別の箇所に影響していることがあるためです。再レビューの差分は小さいので実際の作業量は少なくなります。
...
`thorough-code-review`スキルが起動できない場合は、その旨をPhase 6の報告に記載してスキップします。
...
- `thorough-code-review`スキルは省略しません — 起動できない場合のみ理由を明記してスキップします
