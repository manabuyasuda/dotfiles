...
must/shouldの修正が発生するたびに`code-review`スキルをフルで再実行します。修正が連鎖して別の箇所に影響していることがあるためです。再レビューの差分は小さいので実際の作業量は少なくなります。
...
### Phase 6: ふりかえり（`retrospective`スキル）

`retrospective`スキルを起動し、このセッションのふりかえりを実施します。

```
Skill: retrospective
```

完了条件: `retrospective`スキルが完了していること。
...
### ふりかえり
- ふりかえりファイル: <retrospectiveスキルが更新したファイルのパス>
- Keep: <主なKeep項目>
- Problem: <主なProblem項目>
...
- `code-review`スキルは省略しません — 起動できない場合のみ理由を明記してスキップします
