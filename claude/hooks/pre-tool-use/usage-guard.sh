...
#
# 閾値の所在: 閾値（75% / ¥3,200 など）の判定は statusline.sh に集約してある。
...
if [ "$cost_band" = "red" ] && [ "$prev_cost_warned" != "red" ]; then
  fire="cost"
  new_cost_warned="red"
  msg="STOP: 累計コストが閾値（¥3,200 相当）を超えました。従量課金が際限なく増えるのを避けるため、ここで一度作業を止めます。WHY: コスト上限の歯止めです。FIX: 残作業が本当に必要か・進め方を変えられないかをユーザーと検討し、続行する場合はユーザーに明示的な続行可否を確認してから再開してください。"
elif [ "$ctx_band" = "red" ] && [ "$prev_ctx_warned" != "red" ]; then
  fire="ctx"
  new_ctx_warned="red"
  msg="STOP: コンテキスト使用率が 75% を超えました。auto-compact（95%）を待つと要約の質が落ち応答も劣化します。WHY: 劣化が出る前に圧縮するためです。FIX: keep スキルで重要事項を保存してから /compact を検討するようユーザーに促してください。"
fi
