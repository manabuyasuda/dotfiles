...
if [ -n "$fire_ctx" ]; then
  if [ "$ctx_band" = "red" ]; then
    ctx_msg="コンテキスト使用率が赤帯の閾値に達しました。auto-compact（95%）を待つと要約の質が落ち応答も劣化します。劣化が出る前に keep スキルで重要事項を保存してから /compact を検討するようユーザーに促してください。"
  else
    ctx_msg="コンテキスト使用率が黄帯の閾値に達しました。使いすぎで精度が落ちる前に早めに圧縮し、クリアな状態で作業を続けるため、ここで一度止めます。keep スキルで重要事項を保存してから /compact を検討するようユーザーに促してください。"
  fi
fi
