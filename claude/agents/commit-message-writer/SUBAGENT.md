...
異なる目的の変更が混在している場合は分割します（バグ修正とリファクターを同時に行うときなど）。1つの目的を達成するために必要な変更は同じコミットにまとめます（実装とそのテスト、型定義とその使用箇所など）。
...
1. transcript JSONL — 前回コミット時刻以降のtranscriptから`text`を抽出し、議論の流れ・ユーザーが明示した目的・制約を確認します（手順は後述「transcriptから背景を拾う」を参照）
...
Step 3の情報源1番目で使う手順です。`/compact`をまたいだやり取りや、別セッションでの議論を取りこぼさないために、transcript JSONLから直接拾います。
...
抽出した本文から、ユーザーが述べた目的・制約・判断、採用した方針を「なぜ」候補として整理し、Body案を作る入力にします。要約をそのままBodyに貼らず、diffと突き合わせて該当コミットの範囲に絞った内容だけを採用します。
...
| 規約 | 対応 |
|------|------|
| Subjectは主目的だけ | Subjectは「Git管理外への書き込みを通す」だけに絞っています。文字化け修正はBody末尾に「あわせて」で追記しています |
| 振る舞いベース | 内部処理（stdinの取り出し方・変数展開の挙動）を書かず、観察できる動作を主語にしています |
| 並列は箇条書き | 通すケース・ブロックするケース・境界ケースを並列に並べています |
| 略語禁止 | 「リポ」を使わず「リポジトリ」「Git管理対象外」と書いています |
| 一意な語 | 「リポ外」のような複数の意味に取れる語を避け、「Git管理対象外」と明示しています |
