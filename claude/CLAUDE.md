...
- `retrospective/` — セッションのふりかえり（最新ファイルに随時追記、なければ当日日付で作成）
  ...
  - Try/Action: `/x-retrospective` スキルが追加してファイルに記録する
...
- `/x-thorough-code-review` — PR・ブランチのコードレビュー
- `/x-retrospective` — セッション終了時のKPTA
- `/x-hotspot-refactoring` — リファクタリング優先順位の決定
- `/x-rebasing-feature-branch` — フィーチャーブランチをベースブランチに追従
